...
from piestats.compat import (
    strip_bytes_from_list, strip_bytes_from_dict, strip_bytes_from_stream, kill_bytes)
...
class Results():
  ...
  def get_player(self, player_name):
    ...
    if not player_id:
      return None
    info = strip_bytes_from_dict(self.r.hgetall(self.keys.player_hash(player_id)))
    if not info:
      return None
    info['names'] = self.get_all_names_from_id(player_id)
    return Player(name=self.get_name_from_id(player_id), **info)
  ...
  def get_map(self, map_name, get_svg=False):
    ...
    keys = strip_bytes_from_list(self.r.hkeys(self.keys.map_hash(map_name)))
    ...
    info = self.hmget_with_keys(self.keys.map_hash(map_name), keys)
    ...
    info['plays'] = self.r.zscore(self.keys.top_maps, map_name) or 0
    return Map(name=map_name, **info)
  ...
  def get_player_fields(self, player_id, fields):
    ''' given a player id and some keys, get back a populated Player object '''
    player_id = int(player_id)
    info = self.hmget_with_keys(self.keys.player_hash(player_id), fields)
    return Player(name=self.get_name_from_id(player_id), **info)

  def get_player_top_enemies(self, player_name, startat=0, incr=20):
    ''' given a player name, get list of Player objects for that player, sorted by number of times they killed us desc '''
    player_id = self.get_id_from_name(player_name)
    if not player_id:
      return
    results = self.r.zrevrange(self.keys.player_top_enemies(player_id), 0, startat + incr, withscores=True)
    for enemy_id, their_kills in results:
      enemy_id = int(enemy_id)
      more = self.hmget_with_keys(self.keys.player_hash(enemy_id), ['lastcountry'])
      my_deaths = float(self.r.zscore(self.keys.player_top_enemies(enemy_id), player_id) or 0)
  ...
  def get_player_top_victims(self, player_name, startat=0, incr=20):
    ''' given a player name, get list of Player objects for that player, sorted by number of times we killed them us desc '''
    player_id = self.get_id_from_name(player_name)
    if not player_id:
      return
    results = self.r.zrevrange(self.keys.player_top_victims(player_id), 0, startat + incr, withscores=True)
    for victim_id, my_kills in results:
      victim_id = int(victim_id)
      more = self.hmget_with_keys(self.keys.player_hash(victim_id), ['lastcountry'])
      their_deaths = float(self.r.zscore(self.keys.player_top_victims(victim_id), player_id) or 0)
  ...
  def get_id_from_name(self, name):
    ...
    result = self.r.hget(self.keys.name_to_id, name)
    if result:
      return int(result)
    else:
      return None
  ...
  def hmget_with_keys(self, hash_name, keys):
    ...
    data = strip_bytes_from_list(data)
