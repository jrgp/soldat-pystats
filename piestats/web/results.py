...
# python3 bytes vs str garbage fire
from piestats.compat import (
    strip_bytes_from_stream,
    strip_bytes_from_dict,
    strip_bytes_from_list,
    kill_bytes,
)
...
class Results():
  ...
  def get_top_killers(self, startat=0, incr=20):
    ...
    results = strip_bytes_from_stream(self.r.zrevrange(self.keys.top_players, startat, startat + incr, withscores=True))
    for player_id, kills in results:
      player_id = int(player_id)
      more = strip_bytes_from_dict(
          self.hmget_with_keys(self.keys.player_hash(player_id), ['deaths', 'lastseen', 'firstseen', 'lastcountry', 'scores:Alpha', 'scores:Bravo']))
  ...
  def get_top_maps(self, startat=0, incr=20):
    ...
    results = strip_bytes_from_stream(self.r.zrevrange(self.keys.top_maps, startat, startat + incr, withscores=True))
    for name, plays in results:
      more = strip_bytes_from_dict(
          self.hmget_with_keys(self.keys.map_hash(name), ['scores:Alpha', 'scores:Bravo', 'wins:bravo', 'wins:alpha', 'kills', 'flags']))
      yield Map(name=name,
                plays=plays,
                **more
                )
  ...
  def get_player(self, _player):
    ...
    if not _player_id:
      return None
    info = strip_bytes_from_dict(self.r.hgetall(self.keys.player_hash(_player_id)))
    if not info:
      return None
  ...
  def get_round(self, round_id):
    ...
    info = strip_bytes_from_dict(self.r.hgetall(self.keys.round_hash(round_id)))
    if not info:
      return None
    return Round(round_id=round_id, **info).resolve_players(self)
  ...
  def get_map(self, _map, get_svg=False):
    ...
    keys = strip_bytes_from_list(self.r.hkeys(self.keys.map_hash(_map)))
    ...
    info = strip_bytes_from_dict(self.hmget_with_keys(self.keys.map_hash(_map), keys))
  ...
  def get_player_fields(self, _player_id, fields=[]):
    ...
    info = strip_bytes_from_dict(self.hmget_with_keys(self.keys.player_hash(_player_id), fields))
    return Player(name=self.get_name_from_id(_player_id), **info)
  ...
  def get_player_top_enemies(self, _player, startat=0, incr=20):
    ...
    for enemy_id, their_kills in results:
      ...
      more = strip_bytes_from_dict(self.hmget_with_keys(self.keys.player_hash(enemy_id), ['lastcountry']))
  ...
  def get_player_top_victims(self, _player, startat=0, incr=20):
    ...
    if not _player_id:
      return
    results = strip_bytes_from_stream(self.r.zrevrange(self.keys.player_top_victims(_player_id), 0, startat + incr, withscores=True))
    for victim_id, my_kills in results:
      victim_id = int(victim_id)
      more = strip_bytes_from_dict(self.hmget_with_keys(self.keys.player_hash(victim_id), ['lastcountry']))
  ...
  def get_last_kills(self, startat=0, incr=20):
    ...
    kill_ids = strip_bytes_from_stream(self.r.zrevrange(self.keys.kill_log, startat, startat + incr))
    for kill_id in kill_ids:
      kill_data = strip_bytes_from_dict(self.r.hget(self.keys.kill_data, kill_id))
      if kill_data:
  ...
  def get_last_rounds(self, startat=0, incr=20):
    ...
    round_ids = strip_bytes_from_stream(self.r.zrevrange(self.keys.round_log, startat, startat + incr))
    for round_id in round_ids:
      round_id = int(round_id)
      round_data = strip_bytes_from_dict(self.r.hgetall(self.keys.round_hash(round_id)))
      if round_data:
  ...
  def get_top_weapons(self):
    ...
    results = strip_bytes_from_stream(self.r.zrevrange(self.keys.top_weapons, 0, 20, withscores=True))
    return [(x[0], int(x[1])) for x in results]
  ...
  def get_kills_for_date_range(self, startdate=None, previous_days=7):
    ...
    for x in range(previous_days):
      ...
      try:
        count = int(kill_bytes(self.r.get(key)))
  ...
  def get_top_countries(self, limit=10):
    ...
    return strip_bytes_from_stream(self.r.zrevrange(self.keys.top_countries, 0, limit, withscores=True))
  ...
  def get_all_names_from_id(self, player_id):
    ...
    player_id = int(player_id)
    names_with_scores = list(strip_bytes_from_stream(self.r.zrevrange(self.keys.player_id_to_names(player_id), 0, -1, withscores=True)))
  ...
  def get_id_from_name(self, name):
    ...
    player_id = kill_bytes(self.r.hget(self.keys.name_to_id, name))
    if player_id:
